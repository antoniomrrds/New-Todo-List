...
import { NETWORK_ERROR_MESSAGES_CUSTOM, STATUS_ERROR_MESSAGES } from "@/api/error/status-error-messages";

// Função para tratar os erros da API
export const handleApiError = (error: AxiosError<ApiErrorResponse>, customMessage?: string): FormattedError => {
    let errorMessage = customMessage || NETWORK_ERROR_MESSAGES_CUSTOM.defaultError;  
    ...
    if (error.response) {
      ...
      errorMessage = STATUS_ERROR_MESSAGES[errorCode] || NETWORK_ERROR_MESSAGES_CUSTOM.defaultError;
